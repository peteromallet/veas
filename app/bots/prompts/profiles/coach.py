"""Coach BotProfile — solo career reflection coach.

Populated from the pre-refactor ``SOLO_SYSTEM_PROMPT_V1`` constant
(app/services/prompts_solo.py).

The bot_id is ``'coach'`` (NOT ``'solo_coach'``) — matching the real
bot id at ``app/bots/coach.py:71``.

``{topic_display_name}`` is preserved as a placeholder in ``role_summary``
and substituted via ``**format_kwargs`` by the renderer wrapper.
"""
