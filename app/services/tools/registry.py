...
TOOL_DESCRIPTIONS: dict[str, str] = {
    ...
    "schedule_partner_checkin": "Schedule a future check-in turn on the current user's dyad partner's side, on the same bot. Use ONLY for explicit user requests like 'can you check in on Hannah?' or 'see how my partner is doing.' The partner is resolved server-side from the dyad — you do NOT pass any user id. `nudge_note` is a short neutral note shown to the recipient (never quote the originator). `reason` is audit-only. Optional `bridge_candidate_id` links the nudge to a mediated bridge you created; must be a source-owned bridge (this user is source, partner is target) with status=ready and partner_path=message_partner. Rejects when the recipient has not opted in for this bot (`recipient_state` will be `opt_out` or `pending`), when no dyad partner exists, when a recent nudge already happened in the last 24h, when a pending nudge with the same originator/recipient/bot is still open, or when the bridge is missing/not-owned/wrong-direction/wrong-status. Tell the user what was scheduled — and what you'll say to the partner — in the same reply.",
